...
*===============================================================================
use "$dpath/full_sample_het.dta", clear
gen all = 1
...
*===============================================================================
// STep 2: Follow EBP approach - Hetregress
*===============================================================================
use "$dpath/srs_sample_het.dta", clear
	sort hhid
	hetregress Y_B  x1 x2 x3 x4 x5, het(x) mle
	predict xb, xb
	predict er, sigma
...
use "$dpath/srs_sample_het.dta", clear
	...
	mi set wide
	mi register imputed Y_B
	hetmireg Y_B  x1 x2 x3 x4 x5, het(x) sims(100) uniqid(hhid) errdraw(normal) by(new) lny
	drop if new!=1
	cap drop y_*
	rename yhat* y_*
...
if ($zed==1) save "$dpath/results_micomps_het.dta", replace
else{
	append using "$dpath/results_micomps_het.dta"
	save "$dpath/results_micomps_het.dta", replace
}
