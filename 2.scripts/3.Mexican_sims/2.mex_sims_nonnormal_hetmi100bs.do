...
forval z=501/1000{
	...
	hetmireg `depvar' $themodel [aw=Whh], sims(100) uniqid(hhid) errdraw(empirical) mlong by(_mi_miss)	
	keep if _mi_m>0
	//Get poverty rates
	forval i=5(5)95{
...
if (regexm("`depvar'","8")) save "$dpath\nonnormal8_mex_results_hetmi.dta", replace
else save "$dpath\nonnormal_mex_results_hetmi.dta", replace
